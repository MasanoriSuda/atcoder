...
struct Solver
{
    ...
    int64_t solve()
    {
        ...
        {
            auto [use_card_i, use_target] = select_action(cards);
            ...
            int select_card_i = select_next_card(next_cards, money);
    ...
    pair<int, int> select_action(vector<Card> card)
    {
        int selectval = 0;
        ...
        if (card.size() != 0)
        {
            ...
            {
                if (card[i].t == CardType::INVEST)
                {
                    selectval = i;
                    break;
                }
                /*
                else if (card[i].t == CardType::WORK_ALL)
                {
                    selectval = i;
                    break;
                }
    ...
    int select_next_card(const vector<Card> &next_cards, int money)
    {
        // TODO: implement your strategy
        for (int i = 0; i < next_cards.size(); ++i)
        {
            if (next_cards.at(i).p <= money && next_cards.at(i).t == CardType::INVEST)
            {
                if (zoushi_num < 19)
                {
                    zoushi_num++;
            ...
            else if (next_cards.at(i).p <= money && next_cards.at(i).t == CardType::WORK_ALL)
            {
                return i;
            }
            /*
            else if (next_cards.at(i).p <= money && next_cards.at(i).t == CardType::CANCEL_SINGLE)
            {
                return i;
            }
            else if (next_cards.at(i).p <= money && next_cards.at(i).t == CardType::CANCEL_ALL)
            {
                return i;
            }
            */
